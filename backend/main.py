...
# ── Documents data (from your image) ──────────────────────────────────────────
DOCUMENTS_DATA = {
    "Open": [
        "SSC mark sheet",
        "HSC mark sheet",
        ...
        "Entrance exam scorecard (MHT-CET / JEE / NEET)",
        "Nationality certificate",
        "Aadhar card",
        "Passport size photos (20)",
        "Aadhar linked nationalised bank passbook",
...
def predict_pdf(
    ...
    for i, (_, r) in enumerate(filtered.iterrows(), 1):
        ...
        inst_code = str(int(r['Institution Code'])) if pd.notna(r['Institution Code']) else "N/A"
        data.append([
            str(i),
            inst_code,
            P(r['College Name']),
